...
******************************************/
#include "drp_tvm.h"
TVMModel::TVMModel() : IRecognizeModel(128 * 1024, TVM_MODEL_DIR.data(), MODEL_NAME.data(), TVM_DRPAI_IN_WIDTH, TVM_DRPAI_IN_HEIGHT, TVM_DRPAI_IN_CHANNEL, TVM_MODEL_IN_W, TVM_MODEL_IN_H, TVM_MODEL_IN_C, MODE_TVM_DEEPPOSE_DRPAI)
{
    std::cout << "TVM model" << std::endl;
}

int32_t TVMModel::inf_post_process(float* arg)
{
...
shared_ptr<PredictNotifyBase> TVMModel::get_command()
{
 ...
 * @return int8_t
 */
int8_t TVMModel::post_process(vector<pos_t> &result, float* floatarr)
{
