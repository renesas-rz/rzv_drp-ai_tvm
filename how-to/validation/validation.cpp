 ...
 * Modified Code (C) Copyright Renesas Electronics Corporation 2026
 *
...
* Version      : 1.0.0
* Description  : RUHMI Validation Application
***********************************************************************************************************************/
...
int main(int argc, char **argv)
{
    /* Get argument info */
    auto usage_log = [] {
        std::cout << "Usage: ./validation -m <model dir> [-n <loop_num>] [-f <freq_index>] [-o] [-p] [-h]" << std::endl;
        std::cout << "  -m, --model-dir  <model_dir> : Specific deployment folder" << std::endl;
        std::cout << "  -n, --loop-num   <loop_num> : Number of loops to get average AI processing time. (default 10)" << std::endl;
        std::cout << "  -f, --freq-index <freq-index> : DRP-AI frequency index. Valid only for RZ/V2H and RZ/V2N (default 1)." << std::endl;
        std::cout << "  -o, --output-bin: Speficy this option to save binary output file in <model_dir>." << std::endl;
        std::cout << "  -p, --profilerun: Specify this option to save profile information that shows the layer allocation to DRP-AI/CPU." << std::endl;
    ...
    while (1) {
        static struct option long_opts[] = {
            ...
            {"loop-num", required_argument, 0, 'n'},
            {"freq-index", required_argument, 0, 'f'},
            {"profilerun", no_argument, 0 , 'p'},
            {"output-bin", no_argument,  0 , 'o'},
            {"help",       no_argument,       0, 'h'},
            {0,0,0,0}
        };
        int optidx = 0;
        int c = getopt_long(argc, argv, "m:n:f:poh", long_opts, &optidx);
        if (c == -1) break;
        switch (c) {
            case 'm': {
                std::string v = optarg;
            ...
            case 'f': {
                int v = std::atoi(optarg);
                freq_index = (v < 1) ? 1 : (v > 127) ? 127 : v;
#ifdef V2H
                std::cout << "[INFO] DRP-AI frequency index : " << freq_index << std::endl;
#else
                std::cout << "[INFO] DRP-AI frequency index is not used."<<std::endl;
#endif
                break;
            }
            case 'p': {
                profile_run = true;
                std::cout << "[INFO] Enabled saving profile data." << std::endl;
                break;
            }
            case 'o': {
                output_bin = true;
                std::cout << "[INFO] Enabled saving output binary file." << std::endl;
                break;
            }
            case 'h':
            default:  usage_log(); return 0;
        }
    }
    /* Error check */
    if (model_dir.empty()) {
    ...
    /* Set input data*/
    /* Input binary file must be saved as <model_dir>/input_0.bin file.*/
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        std::cout << "[INFO] Input bin file : " << filename << std::endl;
    ...
    std::cout << "Running tvm runtime for " <<loop_num<<" times" << std::endl;
    ...
    std::cout << "[TIME] AI Processing Time (Avg. over "<<loop_num<<" runs ) : " << std::fixed << std::setprecision(2) << diff << " msec." << std::endl;

    if (profile_run)
    {
        /* Get profile by running inference, again*/
        std::cout << "Getting profile data..." << std::endl;
        std::string profile_table = "./profile_table.txt";
        std::string profile_csv = "./profile.csv";
        runtime.ProfileRun(profile_table, profile_csv);
        std::cout << "[INFO] Profile data is saved as " << profile_table << "_*.txt & "<< profile_csv <<"_*.csv"<< std::endl;
    }
    if (output_bin)
    {
