...
#include "define.h"
#include "define_color_yolox_pose.h"
/*Post process control*/
#include "post_proc_yolox_pose.h"
/*USB camera control*/
#include "camera.h"
/*Image control*/
#include "image_yolox_pose.h"
/*Wayland control*/
#include "wayland.h"
/*box drawing*/
#include "box.h"
...
/*Global Variables*/
static float output_key34_80[num_key34_80];
static float output_key34_40[num_key34_40];
static float output_key34_20[num_key34_20];
static float output_box_80[num_box_80];
static float output_box_40[num_box_40];
static float output_box_20[num_box_20];
static float output_key17_80[num_key17_80];
static float output_key17_40[num_key17_40];
static float output_key17_20[num_key17_20];
static float output_conf[num_conf_80 + num_conf_40 + num_conf_20];
...
static vector<pose_detection> det;
    ...
    for (i = 0;i<output_num;i++)
    {
        ...
        if (InOutDataType::FLOAT16 == std::get<0>(output_buffer))
        {
            ...
            for (int j = 0; j<output_size; j++)
            {
                ...
                {
                    case 0:
                        output_key34_80[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 1:
                        output_key34_40[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 2:
                        output_key34_20[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 3:
                        output_box_80[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 4:
                        output_box_40[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 5:
                        output_box_20[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 6:
                        output_key17_80[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 7:
                        output_key17_40[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 8:
                        output_key17_20[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 9:
                        output_conf[j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 10:
                        output_conf[num_conf_80 + j]=float16_to_float32(data_ptr[j]);
                        break;
                    case 11:
                        output_conf[num_conf_80 + num_conf_40 + j]=float16_to_float32(data_ptr[j]);
                        break;
                    default:
                        break;
                }
        ...
        else if (InOutDataType::FLOAT32 == std::get<0>(output_buffer))
        {
            ...
            for (int j = 0; j<output_size; j++)
            {
                ...
                {
                    case 0:
                        output_key34_80[j]=data_ptr[j];
                        break;
                    case 1:
                        output_key34_40[j]=data_ptr[j];
                        break;
                    case 2:
                        output_key34_20[j]=data_ptr[j];
                        break;
                    case 3:
                        output_box_80[j]=data_ptr[j];
                        break;
                    case 4:
                        output_box_40[j]=data_ptr[j];
                        break;
                    case 5:
                        output_box_20[j]=data_ptr[j];
                        break;
                    case 6:
                        output_key17_80[j]=data_ptr[j];
                        break;
                    case 7:
                        output_key17_40[j]=data_ptr[j];
                        break;
                    case 8:
                        output_key17_20[j]=data_ptr[j];
                        break;
                    case 9:
                        output_conf[j]=data_ptr[j];
                        break;
                    case 10:
                        output_conf[num_conf_80 + j]=data_ptr[j];
                        break;
                    case 11:
                        output_conf[num_conf_80 + num_conf_40 + j]=data_ptr[j];
                        break;
                    default:
                        break;
                }
...
* Description   : Process CPU post-processing for YOLOX Pose
* Arguments     : key34 = key34 output address
                  box = box output address
                  key17 = key17 output address
                  conf = conf output address
* Return value  : -
******************************************/
void R_Post_Proc(float* key34_80, float* key34_40, float* key34_20, float* box_80, float* box_40, float* box_20, float* key17_80, float* key17_40, float* key17_20, float* conf)
{
    /* PrePost */
    float drpai_output_buf[num_inf_out];
    post_proc.PrePost_Proc(key34_80, key34_40, key34_20, box_80, box_40, box_20, key17_80, key17_40, key17_20, conf, drpai_output_buf);
    ...
    for(int i = 0; i < det_buff.size(); i++)
    {
        ...
        for (int j = 0; j < NUM_KPTS; j++)
        {
            spdlog::info("  ID {}: ({}, {}): {}%", j, (int)det_buff[i].kpts[j].x, (int)det_buff[i].kpts[j].y, (round((det_buff[i].kpts[j].c * 100) * 10) / 10));
        }
        iBoxCount++;
    }
...
* Description   : Draw Skeleton on image
* Arguments     : -
* Return value  : -
******************************************/
void draw_skeleton()
{
    vector<pose_detection> det_buff;
    float kpts_c[NUM_KPTS];
    float kpts_x[NUM_KPTS];
    float kpts_y[NUM_KPTS];
    ...
    for (int i = 0; i < det_buff.size(); i++)
    {
        ...
        for (int j = 0; j < NUM_KPTS; j++)
        {
            kpts_c[j] = det_buff[i].kpts[j].c;
            kpts_x[j] = det_buff[i].kpts[j].x;
            kpts_y[j] = det_buff[i].kpts[j].y;
        }
        ...
        for (int j = 0; j < NUM_KPTS; j++)
        {
            if (kpts_c[j] > TH_KPTS)
            {
                float k_x = kpts_x[j];
                float k_y = kpts_y[j];
                img.draw_circle((int32_t)k_x, (int32_t)k_y, kpts_color[j]);
            }
        }

        /* Draw links */
        for (int j = 0; j < NUM_KPTS + 1; j++)
        {
            uint32_t p1 = links_connection[j].first;
            uint32_t p2 = links_connection[j].second;
            float c1 = kpts_c[p1];
            float c2 = kpts_c[p2];

            if (c1 > TH_KPTS && c2 > TH_KPTS)
            {
                float k_x1 = kpts_x[p1];
                float k_x2 = kpts_x[p2];
                float k_y1 = kpts_y[p1];
                float k_y2 = kpts_y[p2];
                img.draw_line_pose((int32_t)k_x1, (int32_t)k_y1, (int32_t)k_x2, (int32_t)k_y2, links_color[j], LINE_THICKNESS);
            }
        }
    }
    return;
...
void *R_Inf_Thread(void *threadid)
{
    ...
    while(1)
    {
        ...
        /*CPU Post-Processing For YOLOX Pose*/
        R_Post_Proc(output_key34_80, output_key34_40, output_key34_20, output_box_80, output_box_40, output_box_20, output_key17_80, output_key17_40, output_key17_20, output_conf);
...
int32_t main(int32_t argc, char * argv[])
{
    ...
    sprintf(time_buf,"logs/%s_app_yolox_pose_cam.log",date_buf);
    ...
    printf("Model : Megvii-Base Detection YOLOX Pose | %s\n", model_dir.c_str());
    printf("Input : %s\n", INPUT_CAM_NAME);
    spdlog::info("************************************************");
    spdlog::info("  RZ/V2H DRP-AI Sample Application");
    spdlog::info("  Model : Megvii-Base Detection YOLOX Pose | {}", model_dir.c_str());
    ...
    {
        /* If not, set the TVM_NUM_THERADS to TVM_NUM_THREADS_DEFAULT */
    ...
    /*Initialize PostProc parameter.*/
    post_proc.init_param();
