   ...
   /* log out */
    ...
    for(i=0;i<NUM_CLASS;i++){
        if(iBitArray[i] > 0){
            ...
            sprintf(strColor,"(RGB:0x%06x)",colors[i]);
            spdlog::info(" Detected Class {} : {} {}", (i),names[i].c_str(),strColor);
        }
    }
...
int32_t main(int32_t argc, char * argv[])
{
    ...
    sprintf(time_buf,"logs/%s_app_deeplabv3_cam.log",date_buf);
    ...
    spdlog::info("  Model : PyTorch DeepLabv3 | {}", model_dir.c_str());
