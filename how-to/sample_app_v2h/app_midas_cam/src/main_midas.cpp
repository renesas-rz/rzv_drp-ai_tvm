...
static uint8_t postproc_data[TVM_MODEL_IN_W * TVM_MODEL_IN_H * NUM_CLASS];
...
/*****************************************
* Function Name : R_Post_Proc_MiDaS
* Description   : CPU post-processing for MiDaS
*
...
void R_Post_Proc_MiDaS(float* floatarr)
{
    ...
    // Convert depth buffer ( class_w x class_h x 1 ) to RGBA depth map ( class_w x class_h x RGBA )
    cv::Mat depth_org(TVM_MODEL_OUT_H, TVM_MODEL_OUT_W, CV_32FC1, floatarr);
    cv::Mat depth_norm;
    cv::normalize(depth_org, depth_norm, 0., 255., cv::NORM_MINMAX, CV_8UC1);
    memcpy(postproc_data, depth_norm.data, TVM_MODEL_OUT_W * TVM_MODEL_OUT_H * NUM_CLASS);
...
int8_t print_result(Image* img)
{
    ...
    int ret = img->draw_depth_map(postproc_data, capture, TVM_MODEL_OUT_W, TVM_MODEL_OUT_H, CAM_RESIZED_WIDTH, CAM_RESIZED_HEIGHT);
    if (ret != 0)
    {
        return -1;
    }

    ret = img->create_side_by_side(CAM_RESIZED_WIDTH, CAM_RESIZED_HEIGHT, CAM_RESIZED_PADDING);
    if (ret != 0)
    {
        return -1;
    }
...
void *R_Inf_Thread(void *threadid)
{
    ...
    while(1)
    {
        ...
        /*CPU Post-Processing For MiDaS*/
        R_Post_Proc_MiDaS(drpai_output_buf);
...
int32_t main(int32_t argc, char * argv[])
{
    ...
    sprintf(time_buf,"logs/%s_app_midas_cam.log",date_buf);
    ...
    Note: If using OpenCV Accelerator v1.10 or later, the OpenCV functionality
          used in this MiDaS sample application does not use DRP-AI 
    ...
    */
    ...
    printf("Model : MiDaS | %s\n", model_dir.c_str());
    printf("Input : %s\n", INPUT_CAM_NAME);
    spdlog::info("************************************************");
    spdlog::info("  RZ/V2H DRP-AI Sample Application");
    spdlog::info("  Model : MiDaS | {}", model_dir.c_str());
