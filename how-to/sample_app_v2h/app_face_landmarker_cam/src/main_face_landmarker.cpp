...
#include "define.h"
#include "define_color_face_landmarker.h"
/*USB camera control*/
#include "camera.h"
/*Image control*/
#include "image_face_landmarker.h"
/*Wayland control*/
#include "wayland.h"
/*Mutual exclusion*/
#include <mutex>
...
* Description   : CPU post-processing for FaceLandmarker
* Arguments     : floatarr = drpai output address
* Return value  : -
******************************************/
void R_Post_Proc(float* floatarr)
{
    vector<Kpt_coords> local_kpts_xy;
    float score;
    float coords_x = 0;
    float coords_y = 0;
    int32_t i;
    ...
    if (score > TH_PROB)
    {
        ...
        {
            coords_x = floatarr[NUM_CONF_SIZE + i * NUM_KPT_COORDS + 0];
            coords_y = floatarr[NUM_CONF_SIZE + i * NUM_KPT_COORDS + 1];

            Kpt_coords kpt;
            kpt.x = coords_x * CAM_IMAGE_HEIGHT + (CAM_IMAGE_WIDTH - CAM_IMAGE_HEIGHT) / 2;
            kpt.y = coords_y * CAM_IMAGE_HEIGHT;
            
            local_kpts_xy.push_back(kpt);
        }
    }
    /* Log Output */
    spdlog::info(" Probability : {} %", (std::round((score*100) * 10) / 10));
    if (score > TH_PROB)
    {
        for (i = 0; i < NUM_KPTS; i++)
        {
            spdlog::info("  ID {} : ({}, {})", i, (std::round(local_kpts_xy[i].x * 100) / 100), (std::round(local_kpts_xy[i].y * 100) / 100));
        }
    }

    mtx.lock();
    kpts_xy.clear();
    copy(local_kpts_xy.begin(), local_kpts_xy.end(), back_inserter(kpts_xy));
    mtx.unlock();
    return;
}

/*****************************************
* Function Name : draw_landmarks
* Description   : Draw Face Connection on image.
* Arguments     : -
* Return value  : -
******************************************/
static void draw_landmarks(void)
{
    ...
    vector<Kpt_coords> local_kpts_xy;
    mtx.lock();
    copy(kpts_xy.begin(), kpts_xy.end(), back_inserter(local_kpts_xy));
    mtx.unlock();

    /* crop box */
    cv::Point p1;
    cv::Point p2;
    p1.x = (CAM_IMAGE_WIDTH - CAM_IMAGE_HEIGHT) / 2;
    p1.y = 0;
    p2.x = CAM_IMAGE_WIDTH - p1.x;
    p2.y = CAM_IMAGE_HEIGHT;
    img.draw_rectangle(p1, p2, rect_color, 4);

    if (local_kpts_xy.empty()) return;

    /* keypoints */
    for (int i = 0; i < NUM_KPTS; i++)
    {
        kpts_x[i] = local_kpts_xy[i].x;
        kpts_y[i] = local_kpts_xy[i].y;
        img.draw_circle((int32_t)kpts_x[i], (int32_t)kpts_y[i], points_color, LINE_THICKNESS);
    }

    /* links */
    for (int i = 0; i < face_connection.size(); i++)
    {
        uint32_t p1 = face_connection[i].first;
        uint32_t p2 = face_connection[i].second;
        float k_x1 = kpts_x[p1];
        float k_x2 = kpts_x[p2];
        float k_y1 = kpts_y[p1];
        float k_y2 = kpts_y[p2];
        img.draw_line_pose((int32_t)k_x1, (int32_t)k_y1, (int32_t)k_x2, (int32_t)k_y2, links_color, LINE_THICKNESS);
    }
    return;
}
...
void *R_Inf_Thread(void *threadid)
{
    ...
    while(1)
    {
        ...
        cv::Mat src = g_frame.clone();
        /* crop */
        int crop_width = CAM_IMAGE_HEIGHT;
        int crop_height = CAM_IMAGE_HEIGHT;
        int x = (src.cols - crop_width) / 2;
        int y = (src.rows - crop_height) / 2;
        cv::Rect roi(x, y, crop_width, crop_height);
        cv::Mat cropped_src = src(roi).clone();
        in_param.pre_in_addr = (uintptr_t)cropped_src.data;
        ...
        /*CPU Post-Processing For FaceLandmarker*/
        ...
        else
        {
            ...
            else
            {
                ...
                if (!inference_start.load())
                {
                    ...
                    g_frame = img.convert_format_Mat(img_buffer0);
...
int32_t main(int32_t argc, char* argv[])
{
    ...
    sprintf(time_buf,"logs/%s_app_face_landmarker_cam.log",date_buf);
    ...
    printf("Model : mediapipe FaceLandmarker | %s\n", model_dir.c_str());
    printf("Input : %s\n", INPUT_CAM_NAME);
    spdlog::info("************************************************");
    spdlog::info("  RZ/V2H DRP-AI Sample Application");
    spdlog::info("  Model : mediapipe FaceLandmarker | {} ", model_dir.c_str());
