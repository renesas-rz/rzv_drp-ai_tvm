...
class Image
{
    public:
        ...
        void draw_circle(int32_t x, int32_t y, cv::Scalar color, uint32_t thickness);
        void draw_line_pose(int32_t x0, int32_t y0, int32_t x1, int32_t y1, cv::Scalar color, uint32_t thickness);
        void draw_rectangle(cv::Point p1, cv::Point p2, cv::Scalar color, uint32_t thickness);
        ...
        cv::Mat convert_format_Mat(uint8_t* img_buf);
    private:
        uint8_t buf_id = 0;
