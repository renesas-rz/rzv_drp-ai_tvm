...
* Arguments     : x = X coordinate of the center of circle
                  y = Y coordinate of the center of circle
                  color = circle color
* Return Value  : -
******************************************/
void Image::draw_circle(int32_t x, int32_t y, cv::Scalar color, uint32_t thickness)
{
    cv::Mat bgra_image(img_h, img_w, CV_8UC4, img_buffer[buf_id]);
    cv::circle(bgra_image, cv::Point(x, y), RADIUS, color, thickness);
}

/*****************************************
* Function Name : draw_line_pose
* Description   : Draw a single line
* Arguments     : x0 = X coordinate of a starting point
*                 y0 = Y coordinate of a starting point
*                 x1 = X coordinate of a end point
...
* Return Value  : -
******************************************/
void Image::draw_line_pose(int32_t x0, int32_t y0, int32_t x1, int32_t y1, cv::Scalar color, uint32_t thickness)
{
    cv::Mat bgra_image(img_h, img_w, CV_8UC4, img_buffer[buf_id]);
    cv::line(bgra_image, cv::Point(x0, y0), cv::Point(x1, y1), color, thickness);
}
...
* Return Value  : -
******************************************/
void Image::draw_rectangle(cv::Point p1, cv::Point p2, cv::Scalar color, uint32_t thickness)
{
    cv::Mat bgra_image(img_h, img_w, CV_8UC4, img_buffer[buf_id]);
    cv::rectangle(bgra_image, p1, p2, color, thickness);
}
...
* Arguments     : in_w = width of current buffered image, which is mainly camera captured image.
...
/*****************************************
* Function Name : convert_format_Mat
* Description   : Convert YUYV image to Mat format
* Arguments     : img_buf = src img buffer (YUYV)
* Return value  : yuyv_image = Mat img (YUYV)
******************************************/
cv::Mat Image::convert_format_Mat(uint8_t* img_buf)
{
#ifdef DEBUG_TIME_FLG
    using namespace std;
    chrono::system_clock::time_point start, end;
    ...
    cv::Mat yuyv_image(img_h, img_w, CV_8UC2, img_buf);
#ifdef DEBUG_TIME_FLG
    end = chrono::system_clock::now();
    double time = static_cast<double>(chrono::duration_cast<chrono::microseconds>(end - start).count() / 1000.0);
