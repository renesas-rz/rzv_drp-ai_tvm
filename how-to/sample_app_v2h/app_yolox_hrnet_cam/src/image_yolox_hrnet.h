...
class Image
{
    public:
        ...
        void draw_line_hrnet(cv::Point point1, cv::Point point2, cv::Scalar color, uint32_t thickness);
        void draw_rect_hrnet(cv::Point point1, cv::Point point2, cv::Scalar color, int32_t thickness);
        Point convertPoint(int originalX, int originalY);
        cv::Mat convert_format_Mat_rgb(uint8_t* img_buf);
