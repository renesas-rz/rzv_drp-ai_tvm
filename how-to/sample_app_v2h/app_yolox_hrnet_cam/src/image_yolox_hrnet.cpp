...
void Image::draw_rect_box(int32_t x, int32_t y, int32_t w, int32_t h, const char * str,uint32_t color)
{
    ...
    /* Check the bounding box is in the image range */
    x_min = x_min < 1 ? 1 : x_min;
    x_max = (((int32_t)out_w - 2) < x_max) ? ((int32_t)out_w - 2) : x_max;
    y_min = y_min < 1 ? 1 : y_min;
    y_max = (((int32_t)out_h - 2) < y_max) ? ((int32_t)out_h - 2) : y_max;
...
/*****************************************
* Function Name : draw_line_hrnet
* Description   : Draw a single line
* Arguments     : point1 = X, Y coordinate of a starting point
*                 point2 = X, Y coordinate of a end point
*                 color = line color
*                 thickness = thickness of line
* Return Value  : -
******************************************/
void Image::draw_line_hrnet(cv::Point point1, cv::Point point2, cv::Scalar color, uint32_t thickness)
{
    cv::Mat bgra_image(out_h, out_w, CV_8UC4, img_buffer[buf_id]);
    cv::line(bgra_image, point1, point2, color, thickness);
}
...
* Return Value  : -
******************************************/
void Image::draw_rect_hrnet(cv::Point point1, cv::Point point2, cv::Scalar color, int32_t thickness)
{
    cv::Mat bgra_image(out_h, out_w, CV_8UC4, img_buffer[buf_id]);
    cv::rectangle(bgra_image, point1, point2, color, thickness);
}
...
/*****************************************
* Function Name : convert_format_Mat_rgb
* Description   : Convert YUYV image to RGB format
* Arguments     : img_buf = src img buffer (YUYV)
* Return value  : rgb_image = converted img (RGB)
******************************************/
cv::Mat Image::convert_format_Mat_rgb(uint8_t* img_buf)
{
#ifdef DEBUG_TIME_FLG
    using namespace std;
    chrono::system_clock::time_point start, end;
    ...
    int img_h = CAM_IMAGE_WIDTH;
#else
    int img_h = CAM_IMAGE_HEIGHT;
#endif
    cv::Mat yuyv_image(img_h, img_w, CV_8UC2, img_buf);
    cv::Mat rgb_image;
    cv::cvtColor(yuyv_image, rgb_image, cv::COLOR_YUV2RGB_YUYV);
#ifdef DEBUG_TIME_FLG
    end = chrono::system_clock::now();
    double time = static_cast<double>(chrono::duration_cast<chrono::microseconds>(end - start).count() / 1000.0);
